const geTranslation = {
  home: {
    welcome:
      "<p>WILLKOMMEN</p> <b>auf der offiziellen Website von Helmchron</b>",
    wel_text:
      "Helmchron ist ein familiengeführtes Unternehmen, das umfassende und vollständige Ingenieurlösungen für Produktionsstätten in den Bereichen Pharmazie, Chemie, Lebensmittel und Bioprodukte, Elektronik, Energie sowie Öl und Gas anbietet. Seit 2010 sind wir stolz darauf, zur Herstellung von weltweit verfügbaren, sicheren und qualitativ hochwertigen Produkten beizutragen. Unsere Mission ist seit unseren Anfängen die gleiche geblieben - wir verbessern die Lebensqualität für alle überall und halten unsere Umwelt für die kommenden Generationen gesund. Heute ist unser Tätigkeitsbereich breiter und unser Team vielfältiger, was einzigartige Perspektiven, Agilität und Effizienz ermöglicht. Unsere Ingenieurlösungen werden nun an führende Unternehmen und Innovatoren in verschiedenen Branchen geliefert, mit Fokus auf globalen Einfluss, nachhaltige Ergebnisse und langanhaltende Partnerschaften."
  },
  who_are_we: {
    title: "",
    text_sub: "",
    history: "",
    history_text: "",
    credo_title: "",
    credo_text: "",
    quotation: ""
  },
  services: {
    title: "",
    sub_text: "",
    icon: {
      title1: "",
      title2: "",
      title3: "",
      title4: "",
      title5: "",
      text1: "",
      text2: "",
      text3: "",
      text4: "",
      text5: ""
    }
  },
  info: {
    title: "",
    reg: "",
    year: "",
    loc: "",
    reg_num: "",
    contact: "",
    phone: "",
    email: "",
    linkedin: ""
  },
  unique: {
    title: "",
    text: ""
  },
  areas: {
    title: "",
    sub_text: ""
  },
  facts: {
    title: "",
    sub_text: "",
    icon_title1: "",
    icon_title2: "",
    icon_title3: "",
    icon_title4: "",
    icon_title5: "",
    icon_title6: ""
  },
  our_line: {
    title: "",
    sub_text: "",
    img_text1: "",
    img_text2: "",
    img_text3: "",
    img_text4: "",
    img_text5: "",
    img_text6: ""
  },
  feedback: {
    title: "",
    text1: "",
    text2: "",
    text3: ""
  },
  news: {
    title: "",
    text: "",
    posts: {
      post1: {
        title: "",
        time: "",
        text: ""
      },
      post2: {
        title: "",
        time: "",
        text: ""
      },
      post3: {
        title: "",
        time: "",
        text: ""
      }
    }
  },
  navbar: {
    who_are_we: "Wer sind wir?",
    services: "Anlagentechnik für kommende Generationen",
    how_do_we_work: "Wie arbeiten wir?",
    home: "Startseite",
    about_us: "Über uns",
    our_team: "Unser Team",
    vision_mission: "Unsere Vision und Mission",
    our_values: "Unsere Werte",
    competences_services: "Kompetenzen & Dienstleistungen",
    our_core: "Unsere Kernkompetenzen",
    our_services: "Unsere Dienstleistungen",
    our_reference: "Unsere Referenzen",
    career: "Karriere",
    news: "Neuigkeiten",
    blog: "Blog",
    contact: "Kontakt",
    legal: "Rechtliche Hinweise",
    departments: "Unsere Abteilungen",
    food: "Lebensmittel & Bio",
    pharma: "Pharma",
    chemicals: "Chemikalien",
    electronics: "Elektronik",
    energy: "Energie",
    oil: "Öl & Gas"
  },
  team: {
    title: "",
    sub_text: "",
    team1: {
      name: "",
      position: "",
      text: ""
    },
    team2: {
      name: "",
      position: "",
      text: ""
    },
    talk: {
      title: "",
      sub: ""
    },
    values_icon: {
      title: "",
      sub_text: "",
      icon1: {
        title: "",
        text: ""
      },
      icon2: {
        title: "",
        text: ""
      },
      icon3: {
        title: "",
        text: ""
      },
      icon4: {
        title: "",
        text: ""
      },
      icon5: {
        title: "",
        text: ""
      }
    },
    text_bottom: "",
    btn_career: "",
    transparency: {
      title: "",
      sub_text: "",
      btn_blue: "",
      btn_orange: "",
      btn_green: ""
    }
  },
  our_core: {
    title: "",
    sub_text: "",
    icon_core: {
      icon1: "",
      icon2: "",
      icon3: "",
      icon4: "",
      icon5: ""
    },
    planning_text: "",
    design_text1: "",
    design_text2: "",
    project_text: "",
    equipment_text: "",
    commissioning_text: "",
    text_book: "",
    unique: ""
  },
  career: {
    title: "",
    sub_text: "",
    opportunities: {
      title: "",
      sub_text: "",
      accordion: {
        title1: "",
        text1: "",
        title2: "",
        text2: ""
      }
    },
    connect: {
      title: ""
    },
    faqs: {
      title: "",
      sub_text: "",
      accor: {
        title1: "",
        text1: "",
        title2: "",
        text2: "",
        title3: "",
        text3: "",
        title4: "",
        text4: "",
        title5: "",
        text5: "",
        title6: "",
        text6: ""
      }
    },
    video_title: ""
  },
  book_btn: "",
  send: "",
  read_more: "",
  video_common: "",
  lets: "",
  quick_links: "",
  contact_us: "",
  download_brochure: "",
  video_common_values: "",
  services_vertical: "",
  btn_references: "",

  vision: {
    title: "",
    sub_text: "",
    our_strategic: {
      title: "",
      text: ""
    },
    our_mission_vision: {
      title: "",
      mission: {
        title: "",
        text: ""
      },
      vision_: {
        title: "",
        text: ""
      }
    }
  },
  our_values: {
    sub_text: ""
  },
  our_way: {
    title: "",
    integrity: {
      title: "",
      text: ""
    },
    commitment: {
      title: "",
      text: ""
    },
    agility: {
      title: "",
      text: ""
    },
    diversity: {
      title: "",
      text: ""
    },
    courage: {
      title: "",
      text: ""
    }
  },
  social_respo: {
    title: "",
    text: ""
  },
  policy: {
    title: "",
    sub1: "",
    text1: "",
    text2: "",
    text3: {
      text: "",
      textI: ""
    },
    text4: "",
    sub2: "",
    text5: "",
    sub3: "",
    text6: "",
    text6ul: {
      li1: "",
      li2: "",
      li3: ""
    },
    text7: "",
    text8: "",
    text9: "",
    text9ul: {
      li1: "",
      li2: "",
      li3: ""
    },
    purposes: {
      sub: "",
      text1: "",
      li1: "",
      li2: "",
      li3: ""
    }
  },
  blog: {
    title: "",
    text: "",
    blog3: {
      title: "",
      text: "",
      content: {
        title: "",
        sub_text: "",
        text1: "",
        references: ""
      }
    },
    blog2: {
      title: "",
      text: "",
      content: {
        title: "",
        sub_text: "",
        text1: "",
        text2: "",
        text3: "",
        references: ""
      }
    },
    blog1: {
      title: "",
      text: "",
      content: {
        title: "",
        sub_text: "",
        text1: "",
        text2: "",
        text3: ""
      }
    }
  },
  chemicals: {
    title: "",
    text1: "",
    text2: "",
    text3: "",
    text4: "",
    text5: "",
    text6: "",
    text7: ""
  }
};

export default geTranslation;